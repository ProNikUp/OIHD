...
sql_p.execute("""CREATE TABLE IF NOT EXISTS prais_p (
				com text
            )""")        
...
def butclick3 ():
	comm=com.get()
	sql_p.execute( 'insert into prais_p VALUES(?)', (comm,))
	conn_p.commit()
...
sql_pl.execute("""SELECT com FROM prais_pl """)
com_pl = sql_pl.fetchone()
...
tk.Label(text=com_pl, fg="black", font=("Arial", 10)) \
...
com=tk.Entry(width=30, font=("Arial", 10))
com.grid(row=9, column=11, columnspan=3)
tk.Button(text="Подтвердить получение", font=("Arial", 12), command=butclick3).grid(row=10, column=11)
